...
**`app.py` is the entire application.** Its only local import is `EXPECTED_SN_VERSION` from `constants.py`. The end-to-end flow:
...
2. **Routing + retrieval**: `DiaryQueryRouter.extract()` asks the query LLM for structured output (`DiarySearchQuery`) to pick a **mode** (`search` vs `summarize`) plus a semantic query and date/tag filters, resolving relative dates and follow-up references (the chat history is passed in via a `MessagesPlaceholder`). `_build_where()` turns the filters into a Chroma `where` clause (date via `$eq`, tag via `{"tags": {"$contains": ...}}` list-membership). If function-calling fails, `_fallback_extract_query()` recovers via regex.
