...
**Dev diagnostics** (`docs/diagnostics.md`): running the app (or the replay below) with `SNCHAT_TRACE=1` appends each turn — extracted `DiarySearchQuery`, retrieval strategy + entries, plan, answer, and the routing log narration — as one JSON line to `diagnostics/traces.jsonl` (`tracing.py`; stdlib-only, gitignored, a no-op without the env var so the offline guarantee holds). `uv run python diagnostics_report.py` builds a self-contained dark-mode HTML viewer (`diagnostics/report.html`; sidebar session→turn, pure-CSS routing pipeline, collapsible scroll-bounded retrieved entries) from those records; `tracing.read_turns()` is the shared reader for it + a future headless LLM-as-judge. `uv run python tests/replay_answer_key.py` replays the mock-diary answer key through live `extract()`+`retrieve()` and reports routing/retrieval discrepancies against ground truth (needs Ollama; never touches `./diary_vector_db`).
...
- **Data contract:** each note's title must begin with an ISO date (`yyyy-mm-dd`), optionally followed by title text. Notes whose title doesn't parse as a date (or whose content is unreadable/encrypted) are skipped and **counted** — the ingest success/error message shows the skip count — and all date sorting/filtering derives from this prefix.
